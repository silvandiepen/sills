---
name: sills-audit-security
description: Perform a non-destructive, report-only security audit of a web, application, iOS, repository, dependency, CI/CD, and software supply-chain surface. Use for source and passive runtime reviews covering authentication, authorization, sessions, input handling, secrets, APIs, headers, dependencies, GitHub Actions, business logic, data exposure, and OWASP-aligned controls. Never use for active exploitation or penetration testing without a separate authorised workflow.
license: MIT
metadata:
...
Use finding IDs `SEC-0001`, `SEC-0002`, and so on.
...
Before writing any Security audit output, read `references/report-contract.md` and use the bundled report template.
...
Recommended status dimensions: Authentication and sessions; Authorization; Input and data handling; Dependencies and supply chain; CI/CD and operations.
